...
export default defineConfig({
    ...
    build: {
        ...
        rollupOptions: {
            ...
            {
                if(source.includes('selectable-items.ts') || source.includes('selectable-items.js') || source.includes('selectable-items.min.js')) { return true; }
            },
            output: {
    ...
    plugins: [{
        ...
        transform(code, id) {
            const libraryMatch = code.match(/['"].*?selectable-items.*?["']/g);
            if(libraryMatch == null) { return null; }
            // console.log(libraryMatch);
            const result = code.replace(libraryMatch[0], '"../libs/selectable-items.min.js"');
            // console.log(result);
            return {
                code: result,
